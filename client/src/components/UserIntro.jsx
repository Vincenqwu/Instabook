...
export default function UserIntro({followName, authInfo}) {
    ...
    useEffect(() => {
        async function getUserInfo(){
            try{
                const response = await fetch(`${process.env.REACT_APP_API_URL}/user/${followName}`, {credentials:'include'});
    ...
    const [followed, setFollowed] = useState(followings.includes(followName));
    const handleClick = async () =>{
        try{
            if(followed){
                await fetch(`${process.env.REACT_APP_API_URL}/user/${followName}/unfollow`, {
                    method: "PUT",
                    credentials: 'include',
                });
            } else{
                await fetch(`${process.env.REACT_APP_API_URL}/user/${followName}/follow`, {
                    method: "PUT",
                    credentials: 'include',
                });
            }
        ...
        <div className="UserIntro">
            <div className="UserIntroWrapper">
                <div className="userIntroLeft">
                    ...
                    <Link to={`/profile/${followName}`} style={{ textDecoration: "none" }}>
                        <img className="userImg" src={followUser?.picture} alt="" />
                    </Link>
                    <Link to={`/profile/${followName}`} style={{ textDecoration: "none" }}>
                        <span className="userName">{followName}</span>
                    </Link>
                </div>
